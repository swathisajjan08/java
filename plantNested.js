...
const stage2Promise = (t1) => {
  return new Promise((resolve, reject) => {
    step3Promise(t1).then((message) => {
      console.log(message);
      fetchFromDev(295).then((rows) => {
        if (rows.length === 0) {
          logPromise("STAGE 2- no such plant id found").then((message) => {
            console.log(message);
            const t2 = Date.now() % 2000;
            step3Promise(t2).then((message) => {
              console.log(message);
              reject({ message: "stage 2 is rejected" });
            });
          });
        } else {
          const row = rows[0];
          logPromise(`STAGE 2(2)- fetching the plant id ${row.id}`).then((message) => {
            console.log(message);
            const t2 = Date.now() % 2000;
            step3Promise(t2).then((message) => {
              console.log(message);
              resolve({ message: "stage 2 is resolved", t2, row });
            });
          });
        }
      });
...
const stage5Promise = (t4, row) => {
  return new Promise((resolve, reject) => {
    step3Promise(t4).then((message) => {
      console.log(message);
      insertIntoMydb(row).then(() => {
        logPromise(`STAGE 5(2)- Inserting the plant id ${row.id}`).then((message) => {
          console.log(message);
        ...
        .then(({ message: message, t2, row }) => {
          ...
          stage3Promise(t2).then(({ message: message, t3 }) => {
            ...
            stage4Promise(t3).then(({ message: message, t4 }) => {
              console.log(message);
              stage5Promise(t4, row).then(({ message: message, t5 }) => {
                console.log(message);
                stage6Promise(t5, row).then(({ message: message }) => {
                  console.log(message);
